...
var genreMovie = function (movies) {
    return Array.from(new Set((movies.map(function (_a) {
        var genre = _a.genre;
        return genre;
    }).flat())));
};
...
var actorsMovie = function (movies) {
    return Array.from(new Set((movies.map(function (_a) {
...
//   return movies.filter((el) => el.title.toLowerCase().includes(name.toLowerCase()))
// }
...
//   return movies.filter((el) => el.title.toLowerCase().includes(string.toLowerCase())
...
// // фильм с id = 2
// const newArrFilterParam = (movies:MyMovies[], key: string, stringAndNumber: string | number)=> {
//   return movies.filter((el) => el[key]===stringAndNumber)
// }
// console.log(newArrFilterParam(movies,'title', "Harry Potter and the Sorcerer's Stone"))
